...
class MyClassifier(torch.nn.Module):
    def __init__(self, featureCount, tagCount, hiddenSize, hiddenCount): # currently one hidden layer, TODO: do it with more layers using an if statement
        ...
        if hiddenCount > 1:
            ...
            self.norm2 = torch.nn.LayerNorm(hiddenSize)
        if hiddenCount > 2:
            self.lin3 = torch.nn.Linear(hiddenSize, hiddenSize)
            self.norm3 = torch.nn.LayerNorm(hiddenSize)
        if hiddenCount > 3:
            self.lin4 = torch.nn.Linear(hiddenSize, hiddenSize)
            self.norm4 = torch.nn.LayerNorm(hiddenSize)
    ...
    def forward(self, batchFeatures, hiddenCount): # also should depend on the number of layers
        ...
        hiddenNorm = torch.sigmoid(self.norm(hidden))
        #print("hidden:", hidden)
        if hiddenCount > 1:
            hidden2 = torch.sigmoid(self.lin2(hiddenNorm))
            hiddenNorm2 = torch.sigmoid(self.norm2(hidden2))
            #print("hidden2:", hidden2)
        if hiddenCount > 2:
            hidden3 = torch.sigmoid(self.lin3(hiddenNorm2))
            hiddenNorm3 = torch.sigmoid(self.norm3(hidden3))
            #print("hidden3:", hidden3)
        if hiddenCount > 3:
            hidden4 = torch.sigmoid(self.lin4(hiddenNorm3))
            hiddenNorm4 = torch.sigmoid(self.norm4(hidden4))
            #print("hidden4:", hidden4)
            
        if hiddenCount == 1:
            return torch.sigmoid(self.linFinal(hiddenNorm))
            #final = torch.sigmoid(self.linFinal(hidden))
            #print("final:", final)
            #return final1
        elif hiddenCount == 2:
            return torch.sigmoid(self.linFinal(hiddenNorm2))
        elif hiddenCount == 3:
            return torch.sigmoid(self.linFinal(hiddenNorm3))
        elif hiddenCount == 4:
            return torch.sigmoid(self.linFinal(hiddenNorm4))
